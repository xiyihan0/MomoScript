...
test("production editor materializes an avatar and restores the authored story after reload", async ({ page }, testInfo) => {
  ...
  if (local) {
    await page.route("https://**/*", async (route) => {
      ...
      if (decodeURIComponent(new URL(url).pathname) === "/ba_kivo/blobs/stickers/alpha/default.avifs") {
        await route.fulfill({
          status: 200,
          body: alphaSequence,
...
test("native notifications show a toast and remain available from the status bar", async ({ page }) => {
  ...
  const notificationsButton = page.getByRole("status").getByRole("button", { name: "通知", exact: true });
  await expect(notificationsButton).toBeVisible();

  await page.getByRole("tab", { name: "本地历史", exact: true }).click();
  await page.getByRole("button", { name: "创建 Checkpoint" }).click();
  const checkpointName = "原生通知 E2E";
  ...
  await expect(checkpointInput).toHaveValue(checkpointName);
  await checkpointInput.press("Enter");

  const notification = page.getByRole("dialog", {
    name: new RegExp(`已创建 Checkpoint：${checkpointName}`)
  });
  await expect(notification).toBeVisible();

  await notificationsButton.click();
  await expect(page.getByRole("list", { name: "通知中心" })).toBeVisible();
  await expect(notification).toBeVisible();
  await page.getByRole("button", { name: "清除所有通知" }).click();
...
test("a second tab blocks writes visibly until explicit takeover", async ({ page, context }) => {
  ...
  await second.goto("/");
  await expect(second.locator("html")).toHaveAttribute("data-mmt-stage", "mmt-ready");

  const readonlyStatus = second.getByRole("status").getByRole("button", { name: /工作区只读/ });
  ...
  let takeoverPrompt = "";
  second.once("dialog", (dialog) => {
    takeoverPrompt = dialog.message();
    void dialog.accept();
  });
  await readonlyStatus.click();
  expect(takeoverPrompt).toBe("接管写入权会使另一个打开此工作区的标签页变为只读。");
...
test("character gallery browses variants and inserts an entity-scoped sticker", async ({ page }, testInfo) => {
  ...
  const thumbnail = variantTile.locator("img.mms-gallery-thumb");
...
test("editor context menu reveals sticker picker for the speaker at cursor", async ({ page }, testInfo) => {
  ...
  await page.keyboard.press("ArrowDown");
  await editor.locator(".view-lines .view-line").nth(1).click({ button: "right" });
  const stickerItemAgain = page.getByRole("menuitem", { name: "插入角色表情差分" });
...
test("local history opens a single-file edit diff and restores it across reload", async ({ page }) => {
  ...
  await expect.poll(() => visiblePreviewText(page), { timeout: 60_000 }).toContain("HISTORY BASELINE");
  ...
  await expect.poll(() => visiblePreviewText(page), { timeout: 60_000 }).toContain("HISTORY CURRENT");
...
test("local history enforces retention and manages paged Checkpoints", async ({ page }) => {
