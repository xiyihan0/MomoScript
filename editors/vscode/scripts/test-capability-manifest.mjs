...
function frozenRecordSource(value) {
  const entries = Object.entries(value).map(([key, item]) => {
    const rendered = JSON.stringify(item, null, 2).replaceAll("\n", "\n  ");
    return `  ${JSON.stringify(key)}: Object.freeze(${rendered} as const)`;
  });
  return `Object.freeze({\n${entries.join(",\n")}\n} as const)`;
}

function runtimeQualificationSource(value) {
  const qualification = Object.fromEntries(value.providers.map(({ key, ...provider }) => [key, provider]));
  return [
    "// Generated by scripts/test-capability-manifest.mjs; do not edit.",
    "",
    `export const GENERATED_TINYMIST_PROVIDER_ARTIFACTS = ${frozenRecordSource(value.artifacts)};`,
    "",
    `export const GENERATED_TINYMIST_PROVIDER_QUALIFICATION = ${frozenRecordSource(qualification)};`,
    ""
  ].join("\n");
}
...
const runtimeQualificationPath = new URL("../../tinymistProviderQualification.generated.ts", fixtureRoot);
const rendered = `${JSON.stringify(manifest, null, 2)}\n`;
const renderedRuntimeQualification = runtimeQualificationSource(manifest);
if (process.env.UPDATE_TINYMIST_CAPABILITY_MANIFEST === "1") {
  await writeFile(checkedPath, rendered);
  await writeFile(runtimeQualificationPath, renderedRuntimeQualification);
} else {
  assert.equal(await readFile(checkedPath, "utf8"), rendered, "checked capability manifest is stale");
  assert.equal(await readFile(runtimeQualificationPath, "utf8"), renderedRuntimeQualification,
    "generated runtime provider qualification is stale");
}
