...
class KivoWikiClient:
    ...
    async def get_bytes(self, url_or_path: str, *, timeout: Optional[float] = None) -> tuple[int, Dict[str, str], bytes]:
        """
        Fetch raw bytes from an absolute URL (https://...) or an API path (/data/...).
        ...
        if not (url_or_path.startswith("http://") or url_or_path.startswith("https://")):
            # Two common cases:
            # - API paths like "/data/students" should resolve under base_url (.../api/v1)
            # - asset paths like "/static/..." often resolve under the host origin (https://api.kivo.wiki)
            if url_or_path.startswith("//"):
                # Scheme-relative URL like "//static.kivo.wiki/...".
                url = f"{self.scheme}:{url_or_path}"
            elif url_or_path.startswith("/data/"):
                url = f"{self.base_url}{url_or_path}"
            elif url_or_path.startswith("/"):
                url = f"{self.origin}{url_or_path}"
            else:
                url = f"{self.origin}/{url_or_path}"
        ...
        try:
            headers = dict(resp.headers)  # type: ignore[arg-type]
        except Exception:
            headers = {}
