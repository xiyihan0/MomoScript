...
use mmt_lsp::position::{LineIndex, MmtClientPosition, PositionConversionError, PositionEncoding};
...
fn shared_unicode_fixture_round_trips_every_position_family() {
    let fixture: Fixture =
        serde_json::from_str(include_str!("fixtures/position-domains.json")).unwrap();
    ...
    for boundary in &fixture.boundaries {
        ...
        let backend = index
            .backend_position(byte, PositionEncoding::Utf16)
            .unwrap();
        assert_eq!(
            backend.into_lsp(),
            Position::new(boundary.line, boundary.utf16)
        );
        let returned = index
            .backend_offset(backend, PositionEncoding::Utf16)
            .unwrap();
        assert_eq!(
            index
                .mmt_position(returned, PositionEncoding::Utf8)
                .unwrap()
                .into_lsp(),
            client.into_lsp()
        );
...
fn retained_generation_lookup_distinguishes_stale_mismatch_and_absence() {
    ...
    let first = service
        .open(uri.clone(), 1, "@typ: #let 中文 = [é 😀]".into())
        .clone();
    ...
    let second = service
        .change(uri.clone(), 2, "@typ: #let 中文 = [é 😀 next]".into())
        .unwrap()
        .clone();
    ...
    assert_eq!(
        store
            .generation(&uri, &first_entry, first_revision)
            .unwrap()
            .revision,
        first_revision
    );
    assert_eq!(
        store
            .generation(
                &uri,
                &Url::parse("untitled:/wrong.typ").unwrap(),
                first_revision
            )
            .unwrap_err(),
    ...
    assert_eq!(
        store
            .generation(&uri, &Url::parse("untitled:/missing.typ").unwrap(), 999)
            .unwrap_err(),
