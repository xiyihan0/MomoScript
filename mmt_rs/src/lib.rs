...
mod tests {
    ...
    fn run_fixture(input_path: &str, typst_mode: bool) {
        ...
        let compiler = compiler::CompilerState::with_options(compiler::CompileOptions { typst_mode, join_with_newline: true });
        let output = compiler.compile(nodes);
    ...
    fn test_fixtures() {
        ...
        let fixtures_text = fs::read_to_string(fixtures_path).expect("Failed to read fixtures.json");
        let fixtures: serde_json::Value = serde_json::from_str(&fixtures_text).expect("Invalid fixtures.json");
        let obj = fixtures.as_object().expect("fixtures.json must be an object");

        for (name, cfg) in obj {
            let typst_mode = cfg.get("typst_mode").and_then(|v| v.as_bool()).unwrap_or(false);
