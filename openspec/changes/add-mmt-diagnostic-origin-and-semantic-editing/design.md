...
Land the renderer implementation as an ordinary commit on the maintained `xiyihan0/tinymist` fork branch, push it, and pin that full commit in `source.revision`. From a clean `TINYMIST_SRC` checkout at that exact revision, run `node editors/vscode/scripts/build-tinymist-artifacts.mjs build-promote`, then invoke `repin` with the version-matching official universal VSIX in `TINYMIST_VSIX`; the owner command runs native/Worker, navigation, and rich-provider qualification atomically. No patch capture/apply step or product submodule participates. Only `repin` may update artifact sizes/SHA, vendored Web package and official-VSIX grammar provenance, native fixture, and `third_party/tinymist/SHA256SUMS`; vendored bytes are never edited manually and repin does not publish a release.
