...
- Web preview 在无文本选择的右键操作上请求 Composer target；interactive MMT SVG 使用 exporter 生成的确定性不透明 label，将气泡、头像、显示名、narration、reply 与 bond 的视觉区域绑定到同一渲染目标。Webview 先校验 labelled SVG 属于当前 DOM render generation，再只在同 token 内吸附到文本位置，并继续仅向 host 发送规范化坐标；token 不作为 freshness 或后端授权证据。Workbench 在点击位置旁通过原生右键菜单展示“连续消息”“从本条起修改显示名”“转到源码”；不可编辑但可导航的 narration 等 authored 内容只展示“转到源码”。显示名使用点击位置旁的 Workbench context view 与原生 InputBox，而不是顶部 Quick Input。UI 不缓存属性或把 label 当作编辑权限。
