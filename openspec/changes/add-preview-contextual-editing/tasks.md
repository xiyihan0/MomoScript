...
- [x] 4.12 Strictly parse reserved semantic SVG labels、treat an unrecognized closest label as a non-text shadow barrier、reject inactive DOM generations and snap surface hits to a stable interior text anchor within the same target token without changing the point-only Webview protocol
...
- [x] 5.10 Browser-drive bubble corners、avatar and display-name semantic hits through full SVG and diff-v1 updates; prove visual parity、label retention、orphan rejection、shadow-label rejection and stale DOM-generation rejection
