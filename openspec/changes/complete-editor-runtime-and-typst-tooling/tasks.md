...
- [ ] 4.13 Implement inlay hints and resolve when advertised
  - Blocker evidence: both fixed artifacts advertise inlay hints, but the checked positive fixture returns `null` from native and no result from Worker. The family and safe resolve route are implemented, but fixed qualification remains `unavailable` and no host registration is published until an effective positive transcript exists. `node ./scripts/test-rich-provider-artifact.mjs native|worker` locks the negative evidence.
