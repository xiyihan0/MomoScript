...
pack-v3 的核心目标是把资源包从“目录约定 + 若干 JSON”升级成“manifest 驱动的资源数据库”。DSL 看到的是稳定的实体、slot、variant 和贡献命名空间；底层可以是 PNG/JPG 文件，也可以是 WebM 帧包、未来的压缩归档或远端缓存。

这里的关键分层是：

- 逻辑层：实体、别名、slot、variant、tags、description、默认值、贡献来源
...
`#1`、`#2` 这类 ordinal selector 必须来自 manifest 明确声明的顺序。不能用文件系统遍历顺序，也不能用自然排序临时推断。
...
```json
{
          ...
          "variants": [
            {
              ...
              "source": { "storage": "dream_stickers_webm", "frame": 0 }
            },
            {
              "id": "dream_002",
              "ordinal": 2,
              ...
              "tags": ["happy", "laughing"],
              "description": "开心大笑的表情。",
              "source": { "storage": "dream_stickers_webm", "frame": 1 }
            }
          ]
        }
      }
  ...
  "storage": {
    ...
    "dream_stickers_webm": {
      "kind": "video-frames",
      "path": "blobs/dream_stickers.webm",
      "container": "webm",
      "codec": "vp9",
      "alpha": true,
      "size": [512, 512],
...
- `id`：稳定机器 id，在同一 subject、同一 contribution namespace、同一 slot 内唯一
- `ordinal`：可选但推荐，用于 `#n` selector，必须 1-based 且唯一
- `names`：作者可直接写出的确定性别名，例如 `梦1`、`>_<笑`
- `tags`：关键词匹配用标签
- `description`：语义查询文本
- `source`：指向 storage 中的图片或帧

`ordinal` 不是文件名编号，也不是数组下标。迁移工具可以从旧文件名或旧 tags 顺序生成它，但生成后必须固化在 manifest 中。
...
- `video-frames`：WebM 帧包，用于压缩大量同尺寸静态表情序列

`video-frames` 的约束：

- 每个 variant 的 `source.frame` 指向一个 0-based 解码帧序号
