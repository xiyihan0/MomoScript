...
Rust core 中的统一结果采用等价的带类型结构，而不是继续传递未解释字符串：

```text
ResolvedResource {
  range,
  render_patch,
  target: Inline | ActorAvatar(actor_id, revision),
  kind: Sticker | Avatar | ScriptAsset | PackAsset | Temporary | WorkspaceFile | RemoteUrl
}
...
`pack_namespace` 与 `storage_id` 必须同时保留，因为 storage id 只在所属 pack 内唯一。
脚本内显式 `@asset` 在解析 `asset::name` 时优先于同名 pack asset；这是文件局部的显式
覆盖，不等同于扩展资源包 contribution 的隐式覆盖。resolver 失败使用独立的
`Resolve` diagnostic phase，并直接指向原始 marker range。
...
cache 与 AVIFS decoder；语言 core 不直接获得这些广泛 I/O 能力。统一 `compile_text`
协调 syntax、mode、actor、asset、resource、resolve、materialize 与 emit，各阶段 diagnostic
保持原始 phase，已经失败的 marker 只由 emitter 生成占位内容，不重复制造错误。
actor revision 的 `avatar:` 也走同一接口；其默认 slot 简写、contribution 简写和完整
avatar 路径在 resolve 阶段归一化，materialized 结果按 `(actor_id, revision)` 绑定。

## First-Revision Decisions

### AVIFS 编码 profile

构建器当前推荐 `aom + AVIFS + q80 + qalpha80 + yuv420 + k30`，但 manifest 不把该 profile 固化为语义常量。storage metadata 必须完整描述实际 codec、alpha、quality、chroma subsampling、frame count、hash 与 random-access capability，使 decoder/cache 可以拒绝不兼容输入。

### 抽帧输出格式

native materializer 第一版优先输出 PNG，以保证 Typst 支持、透明通道和可重复验证；WebP 可作为后续性能优化，不要求在第一版验收中实现。

### manifest 布局

第一版使用单一完整 manifest。按实体拆分 `entities/*.json` 可以在未来增加，但反序列化后的运行时语义必须等价于一个完整 manifest。

### 查询索引

`description` 和 `tags` 可以作为未来关键词或语义索引输入；embedding 只能是可重建派生数据，不能成为资源解析真相。Rust DSL v2 第一版不实现自然语言资源查询。
