...
- `handles` 需要人工或规则审查后导出，参与 DSL 裸名解析
    ...
    "sticker": {
      ...
      "sets": {
        "initial": {
          "display_name": "初始立绘差分",
          "names": ["初始", "default"],
          "storage": "mika_initial_stickers_webm",
          "variants": [
            {
              "id": "initial_001",
              "ordinal": 1,
        ...
        "damaged": {
          "display_name": "战损差分",
          "names": ["战损"],
          "storage": "mika_damaged_stickers_webm",
          "variants": []
        }
      }
    }
...
Kivo `id` 很稳定，但不适合直接作为作者可读资源路径。主资源包需要决定 entity id 是使用拼音/英文 slug、Kivo dev name、还是人工维护的 canonical id。
...
Kivo 的 `nick_name` 经常是逗号分隔字符串，且可能包含大量俗称。迁移工具可以把它们放入 `aliases`，但升级为 `handles` 前需要人工或规则审查。
