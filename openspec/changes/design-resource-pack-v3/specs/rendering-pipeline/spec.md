...
pack-v3 SHALL 使用 manifest 元数据作为实体、贡献、slot、variant 与 storage 引用的权威索引。
...
- AND `sticker/#1` 通过 manifest variant 元数据解析，而不是通过文件系统顺序解析

#### Scenario: 扩展资源包贡献资源但不隐式覆盖

- GIVEN 扩展资源包为 `ba::梦` 贡献 sticker variants
- WHEN base pack 已经为该实体定义默认资源
- THEN 扩展资源包追加 contribution-scoped variants
...
- THEN ordinal MUST 来自 manifest 声明的 `ordinal` 字段或等价的显式 manifest 顺序
