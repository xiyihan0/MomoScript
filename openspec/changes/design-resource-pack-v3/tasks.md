...
- [x] 2.2 明确 variant 通过 `source.storage` / set storage 与 `source.frame` 指向 WebM 中的帧
- [x] 2.3 记录 alpha、codec、hash、frame_count、random_access 等必要元数据
- [x] 2.4 明确进入 Typst 前需要 materialize 为静态图片 cache
- [x] 2.5 明确 WebM blob 推荐按 sticker set 拆分，variant 可继承 set storage

## 3. 后续实现准备

- [x] 3.1 补充 Kivo API 结构笔记，作为重建主资源包的数据源参考
- [ ] 3.2 编写 pack-v2 / Kivo 数据到 pack-v3 manifest 的迁移工具草案
- [ ] 3.3 实测 WebM 编码 profile 在体积、透明度、抽帧速度和画质上的 tradeoff
- [ ] 3.4 设计 resolver 返回的 `ResolvedResource` 数据结构
- [ ] 3.5 设计 CLI / NoneBot / Web-WASM 共用的 materializer 接口
