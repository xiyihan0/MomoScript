...
- **Stable compiler mount and render delta: not promoted by default.** Typst compile p50 improved 34.5%, 50.6%, and 49.1% for small/medium/large; small and large remain below the required 50%. Large p95 improved 20.6%, below the required 35%. Total visual-ready p50 improved only 6.9–8.5%, below 30%. The candidate remains opt-in with `VITE_MMT_PREVIEW_COMPILER_REUSE=1` (or master `VITE_MMT_PREVIEW_INCREMENTAL=1`); the clean compiler oracle is the default and remains explicitly selectable with `VITE_MMT_PREVIEW_INCREMENTAL=0`.
...
- **Tinymist `diff-v1`: skipped.** Pinned Tinymist 0.15.2 exposes no qualified `diff-v1` producer preserving selectable text, debug locations, page identity, and immutable-artifact navigation. The immutable full-artifact renderer remains the only publication path.
