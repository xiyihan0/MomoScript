...
MomoScript 是一个面向 Momotalk / MoeTalk 风格视觉叙事的 DSL 与渲染工具链。当前主要开发主线是 `mmt_rs/` 中的 Rust DSL v2 language core，以及与其直接配套的 Typst v2 façade 和 pack-v3 资源模型。Python DSL 与 legacy JSON renderer 属于历史实现；NoneBot 和 Web 编辑器暂不纳入当前 parser 阶段的交付目标，但不在此宣告永久废弃。
...
- `mmt_nonebot_plugin/`：历史集成，除非单独恢复工作，否则不作为 Rust v2 完成条件
- `web/`：现有编辑器与旧 WASM 集成；当前 parser 阶段暂不修改，后续迁移应单独立项
...
- Typst v2 façade 目标命令：`cd typst_sandbox/mmt_render && typst compile tests/v2-smoke.typ /tmp/mmt-v2-smoke.pdf --root ..`；当前 clean checkout 因缺少 tracked `mmt_options.webp` 尚未满足该检查，见 DSL v2 task 5.6
...
- Python golden、legacy pipeline、NoneBot 和 Web 检查只在明确修改对应历史表面时运行；它们不是 Rust v2 的默认门槛
...
- `openspec/changes/redesign-dsl-syntax-v2/rust-parser-architecture.md`：已实现架构的主说明及剩余 native CLI、真实 Typst compile diagnostic 集成工作
...
- 本阶段不修改 Web 编辑器或承诺旧 `compile_text_*_wasm` ABI 兼容；后续迁移需单独设计和验收。
